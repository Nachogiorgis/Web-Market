import { GamepadDirectional, Inbox, Plus, User, X } from "lucide-react";
...
export function BottomBar({
  ...
  return (
    <div className="fixed bottom-0 left-0 right-0 z-[1000] inline-flex w-screen items-center justify-between gap-2.5 border-t-[0.50px] border-black/15 bg-black/0 px-3 py-3 backdrop-blur-[50px]">
      ...
      <div className="flex items-center justify-center gap-2.5">
        <div className="flex h-7 w-7 items-center justify-center rounded-[10px] p-1 outline outline-[0.50px] outline-offset-[-0.50px] outline-black/20 backdrop-blur-xl hover:bg-black/[2%]">
          {/* <BriefcaseBusiness className="h-4 stroke-black/40" /> */}
          {/* <User className="h-4 stroke-black/40" /> */}
          <Inbox className="h-4 stroke-black/40" />
          {/* <Settings className="h-4 stroke-black/40" /> */}
        </div>
        ...
        <button
          ...
          className="flex h-7 w-7 items-center justify-center rounded-[10px] p-1 outline outline-[0.50px] outline-offset-[-0.50px] outline-black/20 backdrop-blur-xl hover:bg-black/[2%]"
        >
          {isExplorerOpen ? (
            <X className="h-4 stroke-black/40" />
          ) : (
            <GamepadDirectional className="h-4 rotate-45 stroke-black/40" />
          )}
        </button>
      </div>
